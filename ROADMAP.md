...
## v1.9.3 -- Review Desk UX Polish

Improve Review Desk hierarchy, navigation, preview-state clarity, terminology,
Priority Review ordering, and first-viewport usability. Safely normalize
malformed legacy ordering values without changing valid-record behavior.

No schema, analyzer, provider, workflow, persistence-semantic, or product
capability change is part of v1.9.3.

## v1.9.2 -- Documentation, Terminology, and Product Polish

Make the completed product understandable to a first-time user. Rewrite the
README as a product homepage, create task-focused guides, standardize current
terminology, and clearly archive obsolete cleanup/export/plugin-era designs.

No runtime feature, analyzer, provider, sidecar, schema, workflow, or behavior
change is part of v1.9.2.

## v1.9.1 -- Product Hardening and Release Polish

Stabilize the completed v1.9 workflow without adding capabilities. Focus on
robust sidecar updates, accurate candidate terminology, clear diagnostics,
browser validation, documentation consistency, and regression coverage.

No new analyzer, provider, operation, sidecar, schema, workflow, networking,
export, source modification, or execution is part of v1.9.1.
...
## v1.9: Deterministic Local Classical Preview Generation

v1.9 validates the complete preview architecture with one built-in provider:
LOCAL_CLASSICAL. `dataset-forge preview-generate <inspect_output>
<image-reference>` reads one source image for an existing compatible
Improvement Preview record and writes one disposable candidate into the
isolated inspect-output `preview_artifacts/` workspace.

Supported operations are intentionally tiny and conservative:
`REDUCE_HALO` and `REDUCE_ENCODING_ARTIFACTS`. The provider uses Pillow and
NumPy only, records provider/version/operation/parameter provenance, and keeps
candidate identity deterministic from generated bytes.

LOCAL_CLASSICAL is not cleanup, repair, enhancement, AI editing, restoration,
or export. v1.9 does not modify source images or captions, overwrite imported
candidates without explicit `--replace`, create improved datasets, call Krea or
ComfyUI, use networking, subprocesses, ML, diffusion, or add analyzers.
