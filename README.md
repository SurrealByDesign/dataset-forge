...
**v0.6.0-alpha** -- implements the inspect-only foundation for a LoRA Dataset Decision Engine.
...
Public recommendations, cleanup, repair, export, UI, plugins, and analyzer
families beyond the listed first-pass set are not part of this release.
...
v0.6, Dataset Forge does not yet emit public Ready / Review /
Exclude-from-training recommendations.
...
- **No public recommendation command yet.** v0.6.0-alpha exposes inspect-only
  evidence and review guidance. Ready / Review / Exclude-from-training
  recommendations are the next product direction, not current CLI behavior.

- **No cleanup, repair planning, repair, or export.** v0.6.0-alpha is read-only.
